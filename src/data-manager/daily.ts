...
export class DailySessionManager {
  ...
  public add(session: Session): void {
    ...
    if (session.room) {
      this.activeRooms.set(session.room.id, session.room);
      this.orderActiveRooms();
    }
  ...
  private orderActiveRooms(): void {
    const activeRooms = Array.from(this.activeRooms.entries()).sort();
